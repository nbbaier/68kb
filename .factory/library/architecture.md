...
- **SQLite WAL caveat**: `PRAGMA journal_mode = WAL` is not supported for in-memory SQLite databases. In tests using `:memory:`, expect `memory`/non-`wal` and assert WAL behavior only for file-backed DBs.
